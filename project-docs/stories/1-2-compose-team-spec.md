...
- **AD-2 / AD-4 — ports-and-adapters, inward dependencies.** Core (`composer/`) depends only on the port interface; the concrete adapter is never imported by core. Dependency direction `UI → API → core → adapters`; keep `composer/` free of UI/CLI/SDK imports so Epic 2 (UI) and Epic 5 (API/FR-16) can consume it. [Source: ...#AD-2, #AD-4]
