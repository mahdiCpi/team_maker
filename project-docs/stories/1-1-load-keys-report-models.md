...
- 2026-07-25 — `bmad-correct-course` (Epic 1 course correction): Story 0.4 relocated `team_maker/providers/registry.py` → `team_maker/adapters/providers/registry.py` and fixed two catalog data bugs (`GOOGLE_API_KEY`→`GOOGLE_AI_API_KEY`, added `xai`); `keyconfig.py`'s import path updated accordingly. No behavior change to this story's ACs — purely a location/data fix landed by Story 0.4, now live on `develop`/`epic_1` after the Epic 0 merge. **Decision on this story's missing branch history** (flagged in the Epic 0 retrospective): no retroactive `story_1_1` branch will be created. The code predates the CLAUDE.md branching rules (it arrived via the `guru-explore`→`main`→`develop` merge, not a story branch); creating a pointer-only branch now with no unique commits would repeat the exact anti-pattern the retrospective flagged for Stories 0.1-0.4 ("pointer labels attached afterward... not real isolated branches"). Treated as a grandfathered historical exception — status remains `done`; any *future* change to this story's code will get a proper `story_1_1` branch per CLAUDE.md.
